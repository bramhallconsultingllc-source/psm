...
# Optional internal helper (role mix)
...
def select_latest_post_month_before_freeze(anchor_month: int, lead_months: int, freeze_months: set[int]) -> int:
    ...
    # walk backward up to 12 months to find a non-freeze month
    for k in range(0, 12):
...
class ModelParams:
    ...
    freeze_months: set[int]  # typically same as flu months

    # Load zones (budgeted is green cap)
    budgeted_pppd: float
    ...
    flex_cost_multiplier: float  # premium vs base provider loaded cost
    ...
    provider_replacement_cost: float  # per 1.0 provider FTE replaced
    turnover_yellow_mult: float       # extra replacement cost multiplier in yellow zone
    turnover_red_mult: float          # extra replacement cost multiplier in red zone
...
def simulate_policy(params: ModelParams, policy: Policy) -> dict:
    ...
    Simulate a two-level permanent staffing policy over 36 months:
      ...
      - Freeze months: no posting (and no 'winter-base protection'); replace only to annual base outside freeze
      - Supply evolves with turnover + ramp; hires appear when independent-ready (visible)
      - Flex is used as safety stock to keep load near yellow_max (bounded by flex_max_fte_per_month)
      - Economics: SWB/Visit constraint; turnover replacement costs; access risk cost from provider-day gap
    """
    ...
    # Choose per-year winter step posting month (latest non-freeze before anchor-lead)
    # Anchor index: December (or chosen) each year inside the horizon
    anchor_month = int(params.winter_anchor_month)
    anchor_indices = [i for i, m in enumerate(months) if int(m) == int(anchor_month)]

    # We'll attempt a winter step each year
    winter_step_events = []  # (post_idx, vis_idx)
    for a_idx in anchor_indices:
        # choose posting month number and locate its closest index within same year window
        post_m = select_latest_post_month_before_freeze(anchor_month, lead_months, set(params.freeze_months))
        # Determine which index in the timeline corresponds to that month in the same "year" as anchor index
        # If anchor is at index a_idx, then we search backwards up to 11 months for that post_m.
        post_idx = None
        for k in range(0, 12):
            ...
            if 0 <= j < N_MONTHS and int(months[j]) == int(post_m) and int(months[j]) not in set(params.freeze_months):
                ...
                break
        if post_idx is None:
            # as fallback, just post at a_idx - lead_months (even if frozen), but we'll still respect "no posting in freeze"
    ...
    # Supply simulation with cohorts (paid + effective)
    cohorts = [{"fte": max(float(policy.base_fte), 0.0), "age": 9999}]

    supply_paid = [0.0] * N_MONTHS
    supply_eff = [0.0] * N_MONTHS
    start_paid_arr = [0.0] * N_MONTHS
    turnover_shed_arr = [0.0] * N_MONTHS

    # Helper: compute current paid supply from cohorts
    def current_paid() -> float:
        return float(sum(float(c["fte"]) for c in cohorts))

    # Pre-seed: schedule winter steps based on naive expected paid at visibility month
    # We do a first pass supply projection without any winter steps, then compute needed per event, then re-run.
    ...
    # Pass 1: no winter steps yet
    paid_1, eff_1, sp_1, shed_1 = run_supply(hires_visible)

    # Schedule winter steps (visible hires) per year
    for (post_idx, vis_idx) in winter_step_events:
        # Respect posting freeze: if post month is frozen, do not post (and therefore no winter step)
        ...
        step = need * fill_p  # apply fill probability
    ...
    # Replacement-to-base pipeline (Option B): replace ONLY to annual base; postings not allowed during freeze
    # Implement iteratively during a second supply run, using lookahead projection.
    hires_visible_pipeline = hires_visible[:]
    hires_post_month_pipeline = hires_post_month[:]
    hires_reason_pipeline = hires_reason[:]

    # We'll iterate months and schedule base replacements with lead-time awareness.
    # Simple conservative projection: current paid * (1-turnover)^lead + already scheduled at v
    # This is adequate for policy behavior and avoids circular complexity.
    for t in range(N_MONTHS):
        if lead_months <= 0:
            continue
        if t + lead_months >= N_MONTHS:
            continue
        # posting constraints
        ...
        # approximate "current paid at t" from first-pass paid curve including winter steps
        # We use paid_1 which doesn't include winter steps; so recompute paid with current schedule every loop is expensive.
        # Instead, do one recompute after we finish scheduling base replacements using a conservative assumption:
        # use max(paid_1[t], base) as current.
    ...
    # Final supply run with winter steps + base replacement pipeline
    paid, eff, start_paid_arr, turnover_shed_arr = run_supply(hires_visible_pipeline)

    supply_paid = paid
    supply_eff = eff

    # Flex + load + access gap calculations
    # Provider-day equivalents from effective permanent supply
    perm_eff = np.array(supply_eff, dtype=float)
    perm_paid = np.array(supply_paid, dtype=float)
    v_peak = np.array(visits_peak, dtype=float)
    v_avg = np.array(visits_curve_flu, dtype=float)
    dim = np.array(days_in_month, dtype=float)

    prov_day_equiv = np.array([provider_day_equiv_from_fte(f, params.hours_week, params.fte_hours_week) for f in perm_eff], dtype=float)
    prov_day_equiv_safe = np.maximum(prov_day_equiv, 1e-6)

    load_pppd = v_peak / prov_day_equiv_safe  # patients/provider/day equivalent

    # Flex policy: use flex to bring load down toward yellow_max, bounded by max flex
    ...
    for i in range(N_MONTHS):
        ...
        # Required provider-day equiv to hit target_pppd
    ...
    # Provider-day gap relative to yellow_max AFTER flex (for access / lost visits risk)
    # Convert residual excess load to "FTE gap" relative to yellow_max
    ...
    # Amplify replacement cost based on load zone AFTER flex
    ...
    # Flex cost approximation (provider-only premium on provider loaded cost per FTE-year)
    ...
    yellow_cap = float(params.yellow_max_pppd)
    ...
    # Soft penalties (score components) — keep transparent
    # Overstaff penalty: encourage not to wildly overstaff in slow months
    # Here: penalize when load is far below green cap
    under_util = np.maximum((green_cap * 0.70) - load_after_flex, 0.0)  # below 70% of green is "too low"
    overstaff_penalty = float(np.sum(under_util * dim))  # unit = "underload-days"

    # Burnout penalty: nonlinear beyond red_start
    ...
    # SWB/Visit violation penalty (big)
    swb_violation = max(float(annual_swb) - float(params.target_swb_per_visit), 0.0)
    swb_penalty = float(swb_violation) * 1_000_000.0  # scaled to dominate if violated
...
with st.sidebar:
    st.header("Demand")
    visits = st.number_input("Avg Visits/Day (annual baseline)", min_value=1.0, value=36.0, step=1.0)
    annual_growth = st.number_input("Annual Visit Growth %", min_value=0.0, value=10.0, step=1.0) / 100.0
    peak_factor = st.slider("Peak-to-average factor", 1.00, 1.50, 1.20, 0.01)

    st.subheader("Seasonality + Flu")
    seasonality_pct = st.number_input("Seasonality swing % (winter up, summer down)", min_value=0.0, value=20.0, step=5.0) / 100.0
    flu_uplift_pct = st.number_input("Flu uplift % (selected months)", min_value=0.0, value=0.0, step=5.0) / 100.0
    flu_months = st.multiselect("Flu months", options=MONTH_OPTIONS,
    ...
    days_open_per_week = st.number_input("Days Open / Week", min_value=1.0, max_value=7.0, value=7.0, step=1.0)
    ...
    ramp_months = st.slider("Ramp months after independent", 0, 6, 1, 1)
    ...
    budgeted_pppd = st.number_input("Green cap (Budgeted PPPD)", min_value=5.0, value=36.0, step=1.0)
    yellow_max_pppd = st.number_input("Yellow max PPPD", min_value=5.0, value=42.0, step=1.0)
    red_start_pppd = st.number_input("Red starts PPPD", min_value=5.0, value=45.0, step=1.0)

    st.header("Hiring Freeze")
    winter_anchor_month = st.selectbox("Winter anchor month (ready-by)", options=MONTH_OPTIONS, index=11)  # Dec
    ...
    freeze_months = st.multiselect(
        ...
        help="During these months, the model does not post new requisitions (replacement to annual base occurs outside freeze).",
    )
    freeze_months_set = {m for _, m in freeze_months} if freeze_months else set()

    st.header("Flex (Safety Stock)")
    flex_max_fte_per_month = st.slider("Max flex FTE available per month", 0.0, 10.0, 2.0, 0.25)
    flex_cost_multiplier = st.slider("Flex cost multiplier vs base provider", 1.0, 2.0, 1.25, 0.05)
    ...
    provider_replacement_cost = st.number_input(
        ...
        help="Recruiting + onboarding + ramp inefficiency + temp coverage. Used for provider turnover cost only.",
    )
    turnover_yellow_mult = st.slider("Turnover cost multiplier (yellow months)", 1.0, 3.0, 1.3, 0.05)
    turnover_red_mult = st.slider("Turnover cost multiplier (red months)", 1.0, 5.0, 2.0, 0.10)
    ...
    base_min = st.number_input("Base FTE min", min_value=0.0, value=1.0, step=0.25)
    base_max = st.number_input("Base FTE max", min_value=0.0, value=6.0, step=0.25)
    base_step = st.select_slider("Base FTE step", options=[0.05, 0.10, 0.25, 0.50], value=0.25)
    winter_delta_max = st.number_input("Max winter uplift above base (FTE)", min_value=0.0, value=2.0, step=0.25)
    winter_step = st.select_slider("Winter uplift step", options=[0.05, 0.10, 0.25, 0.50], value=0.25)
...
<div class="contract">
  ...
  <ul class="small" style="margin-top:8px;">
    ...
    <li><b>Freeze months:</b> No posting during freeze (typically flu months). Replacements only restore toward <b>annual base</b> outside freeze (Option B).</li>
...
# x ticks: every 3 months for readability
