...
def compute_seasonality_forecast_multiyear(dates, baseline_visits, flu_months, flu_uplift_pct):
    ...
    """
    flu_set = set(int(m) for m in (flu_months or []))
    raw = []
    for d in dates:
...
def apply_visits_noise(visits_curve, cv):
    """
    ...
    # lognormal parameters: if X ~ LogN(mu, sigma^2), CV relates to sigma.
    sigma = math.sqrt(math.log(1.0 + cv ** 2))
    # Set mu so mean multiplier is 1.0
    mu = -0.5 * sigma * sigma
    multipliers = np.random.lognormal(mean=mu, sigma=sigma, size=len(visits_curve))
...
def simulate_supply_multiyear(
    ...
    confirmed_hire_index=None,          # <<< ABSOLUTE INDEX (preferred)
...
if run_model:
    ...
    total_lead_days_base = int(days_to_sign + days_to_credential + onboard_train_days + coverage_buffer_days)
    ...
    strategy_base = auto_freeze_strategy_v3_from_typical(
    ...
    req_post_month_base = int(strategy_base["req_post_month"])
    hire_visible_month_base = int(strategy_base["hire_visible_month"])
    independent_ready_month_base = int(strategy_base["independent_ready_month"])
    freeze_months_base = list(strategy_base["freeze_months"])
    recruiting_open_months_base = list(strategy_base["recruiting_open_months"])
    lead_months_base = int(strategy_base["lead_months"])

    # Derived ramp: gap at flu start (typical) / months in flu window
    flu_month_idx_typical = int(flu_start_month) - 1
    months_in_flu_window = max(len(strategy_base["flu_months"]), 1)
    ...
    if not enable_probability:
        ...
        # Truth test (safe, after variables exist)
        hire_month = confirmed_hire_month_int
        hire_fte = float(confirmed_hire_fte)
        display_months = [d.month for d in dates_12]
        hire_points = [(d.strftime("%Y-%m"), float(s)) for d, s in zip(dates_12, supply_rec_12) if int(d.month) == hire_month]
        ...
        # Monte Carlo
        # -------------------------
        q_supply = float(1.0 - confidence_level)  # "near-certain supply" = lower quantile
        q_target = float(confidence_level)        # "near-certain demand/target" = upper quantile

        runs = int(mc_runs)
        n = len(dates_full)

        # Store only the display-window slices to keep memory bounded
        supply_rec_runs_12 = np.zeros((runs, 12), dtype=float)
        supply_lean_runs_12 = np.zeros((runs, 12), dtype=float)
        target_runs_12 = np.zeros((runs, 12), dtype=float)
        lean_runs_12 = np.zeros((runs, 12), dtype=float)
        visits_runs_12 = np.zeros((runs, 12), dtype=float)
        ...
        for r in range(runs):
            ...
            tv = float(turnover_var)
            turnover_r = float(provider_turnover) * (1.0 + np.random.uniform(-tv, tv))

            # pipeline duration variation (run-to-run) -> affects lead months and blackout
            pdv = int(pipeline_var_days)
            total_lead_days_r = max(0, int(total_lead_days_base + np.random.randint(-pdv, pdv + 1)))
            ...
            idx = np.array(display_idx, dtype=int)
