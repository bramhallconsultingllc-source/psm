...
def monte_carlo_quantiles(
    ...
    for _ in range(int(mc_runs)):
        ...
        # Strategy rebuilt from typical curve (so pipeline uncertainty can shift req/hire months)
    ...
    def qcurve(runs, q):
        arr = np.array(runs, dtype=float)  # shape: (runs, 12)
